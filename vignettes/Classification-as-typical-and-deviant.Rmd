---
title: "Classification-as-typical-and-deviant"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Classification-as-typical-and-deviant}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
library(MMRcaseselection)
```

The classification (designation, assignment) of cases as typical and deviant (or another) type needs to be distinguished from the actual choice of cases belonging to a specific type. The package offers two ways of classifying cases. First, one can designate cases using an % *prediction interval* or the *standard deviation of the residuals*. Both classification techniques require an `lm` object as input. 

The approach using the prediction interval additionally requires the specification of the width of the interval. The % prediction interval represents the range of outcome values within which 95% of future outcome values are expected to fall. Following Rohlfing and Starke, a case is designated as typical if the observed outcome value is inside the interval and as deviant otherwise. Here is a simple example using the in-built `mtcars` dataset and a 90% predidction interval. 

The output of the function is a dataframe with the observed and fitted outcome value, the upper and lower bound of the prediction interval and the case status based on the fitted value and the prediction interval. The plot is a `gg` object and can be edited with the usual commands of the `ggplot2` package.
```{r}
df <- lm(mpg ~ disp + wt, data = mtcars)
pi_df <- predint(df, piwidth = 0.9)
pi_df
```

The designation of ccases based on the prediction interval can be visualized with the `predint_plot()` command. The graph plots observed vs. fitted outcome values together with the % prediction interval. The input has to be a dataframe produced with `predint()`. Each case has its own interval because they can differ between cases.
```{r, fig.height=6,fig.width=6}
predint_plot(pi_df)
```

An alternative classification technique uses the *standard deviation of the residuals*. The command for assigning cases and typical and deviant is `resid_std()`. The input is an `lm` object and `stdshare` determines how large the share of the residual standard deviation should be to dinstinguish typical frm deviant cases.
```{r}
resid_df <- resid_std(df, stdshare = 1.5)
resid_df
```

To my knowledge, it has not been proposed to plot the case classification resulting from this technique. However, it can be done and is available in this package with the command `residstd_plot()`. The standard deviation of the residual is necessarily the same for all cases and is represented by a symmetric corridor around the bisecting line in a plot of observed vs. fitted outcome values.
```{r, fig.width = 6, fig.height = 6}
residstd_plot(resid_df)
```

## Packages used in this vignette
-   base (R Core Team 2020)
-   ggplot2 (Wickham 2016)
-   grateful (Rodriguez-Sanchez 2018)

R Core Team. 2020. *R: A Language and Environment for Statistical
Computing*. Vienna, Austria: R Foundation for Statistical Computing.
<https://www.R-project.org/>.

Rodriguez-Sanchez, Francisco. 2018. *grateful: Facilitate Citation of R Packages*.
<https://github.com/Pakillo/grateful>.

Wickham, Hadley. 2016. *ggplot2: Elegant Graphics for Data Analysis*.
Springer-Verlag New York. <https://ggplot2.tidyverse.org>.


